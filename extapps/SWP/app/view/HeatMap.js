Ext.define('SWP.view.HeatMap', {
 ...
 eachYValue: function(record, fn, scope) {
        ...
        var me = this,
            ...
            i, ln, accessor;
        yValueAccessors =[].concat(me.yField);
        for (i = 0, ln = yValueAccessors.length; i < ln; i++) {
            accessor = yValueAccessors[i];
            
            fn.call(scope, record.get(accessor), i);
        }
    },
        ...
        var me = this,
            ...
            i, ln, record,
            stacked = me.stacked,
            min=0, max=0,origMin=0,origMax=0,
            positiveTotal, negativeTotal;

        function eachYValueStacked(yValue, i) {
            if (!me.isExcluded(i)) {
                if (yValue < 0) {
                    negativeTotal += parseFloat(yValue);
                } else {
                    positiveTotal += parseFloat(yValue);
                }
            }
        }
        ...
        function eachYValue(yValue, i) {
            ...
            if (!me.isExcluded(i)) {
                ...
                if (yValue < min) {
                    min = parseFloat(yValue);
                }
            }
        }
        function origEachYValue(yValue, i) {
             yValue = parseFloat(yValue);
            if (!me.isExcluded(i)) {
                if (yValue > origMax) {
        ...
        if (count > 0) {
            ...
            for (i = 0, ln = data.length; i < ln; i++) {
                ...
                if (stacked) {
                    ...
                    me.eachYValue(record, origEachYValue);
        ...
        return [min, max,origMin,origMax];
    },
...
//         console.log(bbox.width + "--bbox.width===" +  padding.width +"-- maxy-mingy ---" +maxY +"-- miny--"+ minY );
...
//            console.log(bbox.width + "--bbox.width===" +  padding.width +"-- plus ---" +plus +"-- minus--"+ minus );
           //scale =1;
           if(scale==Infinity){
            scale =1;
           }
    ...
    // @private
    getFillColor: function(val, totalVal,maxValue) {
        var idx = 0;
        ...
        if (val == 0) {
           ...
           idx =this.palette.length -1;
             ...
             idx = Math.floor(32*(val/totalVal));
        }

       
        
        if( idx > 32 ){
        	return this.palette[32];
        }
        // console.log(idx);
    ...
    getPaths: function() {
        ...
        for (i = 0, total = data.length; i < total; i++) {
            ...
            for (j = 0, counter = 0; j < barsLen; j++) {
               ...
               // height = Math.round((me.defaultSize - mmax(bounds.minY, 0)) * bounds.scale);

               height = Math.round( (bounds.bbox.width - bounds.padding.width )/bounds.bars.length );
//               console.log(height);
                idx = themeIndex + (barsLen > 1 ? j : 0);
//                console.log( "idx == " + idx +" height ===" + height + "minY ==" + bounds.minY+"--scale=="+bounds.scale);
                var c = me.getFillColor(yValue, me.totalHits,bounds.origMax);

                // console.log('COLOR', c);

                barAttr = {
                    fill: c //colors[idx % colorLength]
                };
                if (column) {
                    Ext.apply(barAttr, {
                        height: height,
