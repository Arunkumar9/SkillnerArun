...
Ext.define('SWP.view.SeriesScore', {
    ...
    maxBarsCount: undefined,

    showBars: "both",

    // colorArrayStyle: ["#6a6a6a"],
    ...
    getGutters: function() {
        ...
        return {
            ...
            barWidth: me.getBounds().groupBarWidth
        };
    },
    ...
    getBounds: function() {
        ...
        //     groupGutter,
        //     groupBarWidth,
        //     ((stacked ? 1 : groupBarsLen) * (groupGutter + 1) - groupGutter)
    ...
    getPaths: function() {
        var me = this,
            ...
            yFields = Ext.isArray(me.yField) ? me.yField : [me.yField],
             xFields = Ext.isArray(me.xField) ? me.xField : [me.xField],
            gutter = me.gutter / 100,
            groupGutter = me.groupGutter / 100,
            ...
            shadowIndex, shadow, sprite, offset, floorY, idx, circleRadius;
        ...
        for (i = 0, total = data.length; i < total; i++) {
            ...
            for (j = 0, counter = 0,dummyCounter=0; j < barsLen; j++) {
                // Excluded series
                if (me.__excludes && me.__excludes[j]) {
                    continue;
                }
                yValue = record.get(bounds.bars[j]);
                   xValue = record.get(bounds.xBars[j]);
                if (yValue >= 0) {
               ...
               // console.log(colors);
                barAttr = {
                    fill: "#6a6a6a" //colors[idx % colorLength]
                };

                if (column) {

                    circleRadius = me.initialcircleRadius ?  me.circleRadius  : (mmax(bounds.groupBarWidth/3, 0) > me.circleRadius ?  mmax(bounds.groupBarWidth/3, 0) : me.circleRadius );

                    Ext.apply(barAttr, {
                        height: height,
                        width: mmax(bounds.groupBarWidth, 0),
                        // x: (bbox.x + padding.left  +
                        //     ( ( (i +1) * barsLen * mmax(bounds.groupBarWidth, 0) )- (dummyCounter * mmax(bounds.groupBarWidth, 0) )) *(1 + groupGutter)),
                        // x:(bbox.x + padding.left  +
                        //     ( ( (i +1) * barsLen * ( mmax(bounds.groupBarWidth, 0) - (1 + groupGutter) ) )- (dummyCounter * mmax(bounds.groupBarWidth, 0) )) * (1 + groupGutter)),
                        // x: (me.boundColumn ? bounds.barsLoc[i]
                        //                    : (bbox.x + padding.left
                        //                         + (barWidth - shrunkBarWidth) * 0.5
                        //                         + i * barWidth * (1 + gutter)
                        //                         + (yValue >= 0 ? (  barsLen- dummyCounter -1) : 0 ) * bounds.groupBarWidth * (1 + groupGutter) * !stacked)),
                        x: (me.boundColumn ? bounds.barsLoc[i]
                                           : (bbox.x + padding.left
                                                + (barWidth - shrunkBarWidth) * 0.5
                                                + i * barWidth * (1 + gutter)
...
//                         // bbox.x,
//                        // padding.left,
//                        // mmax(bounds.groupBarWidth, 0),
//                        // total,
//                        barsLen,   
//                        counter ,    dummyCounter,
//                       // i,
//                       (me.boundColumn ? bounds.barsLoc[i]
//                                           : (bbox.x + padding.left
//                                                + (barWidth - shrunkBarWidth) * 0.5
//                                                + i * barWidth * (1 + gutter)
//                                                + (yValue >= 0 ? (  barsLen-dummyCounter ) : 0 ) * bounds.groupBarWidth * (1 + groupGutter) * !stacked))
//                        // (bbox.x + padding.left  +
//                        //     ( ( (i +1) * barsLen * mmax(bounds.groupBarWidth, 0) )- (dummyCounter * mmax(bounds.groupBarWidth, 0) )) *(1 + groupGutter)),
//                        //  (bbox.x + padding.left  +
//                        //     ( ( (i +1) * barsLen * ( mmax(bounds.groupBarWidth, 0) - (1 + groupGutter) ) )- (dummyCounter * mmax(bounds.groupBarWidth, 0) )) )
//                        ,barWidth,
//                        shrunkBarWidth,
//                        gutter,
//                        counter,
...
//                        );
                ...
                if (j == (barsLen - 1)) {
                    ...
                    // cirX = bounds.groupBarWidth/2 + (me.boundColumn ? bounds.barsLoc[i]
                    //                    : (bbox.x + padding.left
                    //                         + (barWidth - shrunkBarWidth) * 0.5
                    //                         + i * barWidth * (1 + gutter)
                    //                         + (  barsLen -1) * bounds.groupBarWidth * (1 + groupGutter) * !stacked));
                cirX = bounds.groupBarWidth/2 + (me.boundColumn ? bounds.barsLoc[i]
                                       : (bbox.x + padding.left
                                            + (barWidth - shrunkBarWidth) * 0.5
                                            + i * barWidth * (1 + gutter)
            ...
            // if (!Ext.isEmpty(circleAttr)) {
                    var cyValue = record.get(me.answerField);
                // console.log('SCALE', bounds.scale, cyValue, Math.floor(cyValue*bounds.scale));
                    var x = {
                        radius: Math.floor(circleRadius),
                        fill: SWP.theme =='Dark' ? '#5a5a5a' : '#5a5a5a',
                        x: Math.floor(cirX),
                        y: bottom - Math.floor(cyValue*bounds.scale),
                        id :  'quizBlueCircle'+i
                    };

                    if( record.get(me.answerField) != null ){
                        items.push({
                            type: 'circle',
                            series: me,
                            yField: me.answerField,
                            storeItem: record,
                            // value: [record.get(me.xField), cyValue],
                            attr: x,
                            point: [x.x, x.y]
                        });
                    }

                   // sprite = group.getAt(barsLen);
                   // console.log('GROUP',i,barsLen,j,group, sprite);
                   //  if (sprite) {
                   //      console.log('applying attribute', x);
                   //      sprite.setAttributes(x, true);
                   //  }
            // }
    ...
    drawSeries: function() {
        ...
        for (i = 0; i < ln; i++) {
            ...
            if (!sprite) {
                if (!items[i].type || items[i].type != 'circle') {
                // console.log('CREATING SPRITE!!', baseAttrs, barAttr);
                attrs = Ext.apply({}, baseAttrs, barAttr);
                // console.log('ATTRSSSS', attrs);
                } else {
                    attrs = Ext.apply({}, barAttr);
                }
                attrs = Ext.apply(attrs, endSeriesStyle || {});

                // console.log('ATTRSSSS1', attrs);
                sprite = surface.add(Ext.apply({}, {
                    type: items[i].type ? items[i].type : 'rect',
                    group: group
                }, attrs));

                // console.log('SPRITE1', sprite);
            }
            if (animate) {
    ...
    showTip: function(item) {
        var me = this,
            ...
            x,
            y;
        if (!me.tooltip || item.type =='circle') {
            return;
        }
