...
Ext.define('SWPCommon.view.BadgeTab',{
 extend: 'SWPCommon.view.BadgeButton',
alias:'widget.badgetab',
...
requires: [
        ...
        setCard: function(card) {
            ...
            me.setBadgeText( card.badgeText || me.badgeText );
            me.setIconCls(me.iconCls || card.iconCls);
            me.setIcon(me.icon || card.icon);
            me.setGlyph(me.glyph || card.glyph);
			//201311171420
            me.setToolTip(me.taskStatusId || card.taskStatusId);
        },
        
        /**
         * 201311171420
         * Prepare the tooltip text, for the quiz status icon present in the quiz tab
         * 
         * */
        setToolTip : function( val, fromUpdate ){
        	if( val ){
        		 var cssRecord = this.taskActionStore.findRecord('ID',parseInt(val));
        		 if (cssRecord){
        			 
        			 this.tooltip1 = cssRecord.get('QuizTabTooltip');//QuizTabTooltip
 			    	var statusId = cssRecord.get('ID');
 			    		//201311301937
 			    		if ( statusId == "5" ){
 			    			
 			    			if( !SWP.instructLogin ){
 			    				this.tooltip1 = TASKSTATUS.QUIZ_TAB_TOOLTIP_TASKSTATUS_YOU;
 			    			}
 			    		}
       ...
       /**
        ...
        updateBadge:function( text , appendToExisting){
        	
        	var oldBadgeText = this.badgeText || '';
        	if( appendToExisting ){
        		text = oldBadgeText + text;
        	}
        	return this.setBadgeText( text );
        }
    });