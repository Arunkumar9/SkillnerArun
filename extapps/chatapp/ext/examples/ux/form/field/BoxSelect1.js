...
Ext.define('Ext.ux.form.field.BoxSelect', {
	...
	initComponent:function() {
		...
		this.selectedRecordsStore = Ext.create('Ext.util.MixedCollection');
		...
		this.store.on('datachanged', function(store){
				...
				this.store.each(function(rec)
					{
						...
						this.selectedRecordsStore.add((rec.data[this.valueField]),rec);
					}, this);
			...
			if(!this.recSelected)
			{
				...
				if(this.getCursorPosition()>0)
				{
						...
						if((this.inputEl.dom.value.substring(0,((this.getCursorPosition())-1))) != this.prevVal || (this.inputEl.dom.value.length) < (this.prevLen))
						{
							this.reloadRec();
						}
						this.prevVal=this.inputEl.dom.value;
						this.prevLen = this.inputEl.dom.value.length;
						this.selectedRecordsStore.each(function(rec)
						{
							if(rec.get(this.valueField).substring(0,this.inputEl.dom.value.length) === this.inputEl.dom.value)
							{
							...
							{
								this.selectedRecordsStore.remove(rec);
							}
						}, this);
	...
	reloadRec :function()
	{
		this.selectedRecordsStore.clear();
...
Ext.define('Ext.ux.BoxSelect.Item', {
	...
	renderTpl: [' <span>{caption}</span><div class="deletebutton"><!-- --></div> '], 
  ...
  beforeRender: function(){
    	var me = this;
         ...
         Ext.applyIf(me.renderData, {caption: this.caption});
    },
	onRender: function(ct, position){
