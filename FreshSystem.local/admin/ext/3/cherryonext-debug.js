...
// $Id: cherryonext-debug.js 2348 2013-12-20 10:20:54Z arun $
...
Ext.extend(Ext.ux.netbox.core.Field,Ext.util.Observable,
{
  ...
  setAvailableValues: function(store){
    //Fresh.console.log(store);
    if ('string' == typeof store) {

        var tmp = Ext.StoreMgr.lookup(store);
        if (!tmp)
            tmp =  new Fresh.data.ViewProvider({
                id: store
            })
        store = tmp;
    }
    ...
    if (store)
        Fresh.console.log("FILTER: Store in "+this.id+" undefined");
