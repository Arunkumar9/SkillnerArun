...
// -------- helper
    ...
    {
        var formValues = {};
		var cards = this.cardPanel.items;
		for (var i = 0, len = cards.getCount(); i < len; i++) {
		    if (cards.itemAt(i).form) {
		        formValues[cards.itemAt(i).id] = cards.itemAt(i).form.getValues(false);    
		    } else {
		        formValues[cards.itemAt(i).id] = {};
		    }
		}
	...
	initEvents : function()
	{
		...
		for (var i = 0, len = cards.getCount(); i < len; i++) {
			cards.itemAt(i).on('show', this.onCardShow, this);
			cards.itemAt(i).on('hide', this.onCardHide, this);
			cards.itemAt(i).on('clientvalidation', this.onClientValidation, this);
		}
	},
...
// -------- listeners	
	...
	{
		if (this.fireEvent('previous', this, this.getWizardData(), this.currentCard) !== false) {
	...
	{
		if (this.fireEvent('next', this, this.getWizardData(), this.currentCard) !== false) {
