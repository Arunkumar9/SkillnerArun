...
class FAttributeReader
{
	...
	protected function __construct($attributes)
	{
		...
		foreach($attributes as $key => $item)
			if (is_array($item))
				$this->attributes[$key]=new FAttributeReader($item);
	}
	 ...
	 * @return FAttributeReader An attribute reader instance
	 */
	
	public static function MethodAttributes($class,$method)
	{
		$method=new ReflectionMethod($class,$method);
		$yaml=FAttributeReader::ParseDocComments($method->getDocComment());
		
		return new FAttributeReader(syck_load($yaml));
	}
	...
	public static function ClassAttributes($class)
	{
		...
		$yaml=FAttributeReader::ParseDocComments($class->getDocComment());
	...
	public static function PropertyAttributes($class,$prop)
	{
		...
		$yaml=FAttributeReader::ParseDocComments($prop->getDocComment());
