 ...
 * </ul>
 * @version   1.1.1
 * @revision  $Id: Ext.ux.grid.Search.js 2348 2013-12-20 10:20:54Z arun $
 *
...
Ext.extend(Ext.ux.grid.Search, Ext.util.Observable, {
	 ...
	 */ 
	,searchTipText: __('Type a text to search and press Enter')
	 ...
	 */
	,minCharsTipText: __('Type at least {0} characters')
	...
	,init:function(grid) {
		this.grid = grid;
                this.viewContext = this.viewContext || 'search-'+grid.id;
		...
		if('string' === typeof this.toolbarContainer) {
			this.toolbarContainer = Ext.getCmp(this.toolbarContainer);
                ...
                var store = this.store || this.grid.store;
                if (store.lastOptions && store.lastOptions.params) {
                        delete(store.lastOptions.params[this.paramNames.fields]);
                        delete(store.lastOptions.params[this.paramNames.query]);
                }
                if (store.baseParams[this.paramNames.query]) {
                    delete(store.baseParams[this.paramNames.query]);
                    delete(store.baseParams[this.paramNames.fields]);
                    store.reload();
                }
//                delete(this.grid.store.baseParams[this.paramNames.fields]);
//                delete(this.grid.store.baseParams[this.paramNames.query]);
		// do our processing after grid render and reconfigure
		grid.onRender = grid.onRender.createSequence(this.onRender, this);
                if ('function' === typeof grid.reconfigure )
	...
	,onRender:function() {
		...
		// add menu button
		if(this.showMenu === true || this.showMenu === undefined){
			tb.add({
	...
	,onTriggerClear:function() {
		if(this.field.getValue() || this.field.getValue() == "") {
			this.field.setValue('');
			this.field.focus();
			this.onTriggerSearch();
		}
                    ...
                    if( store.baseParams['tags'] ){
        				delete(store.baseParams['tags']);
        			}
                    if( store.baseParams['parent_id'] ){
        				delete(store.baseParams['parent_id']);
        			}
                }
	...
	,onTriggerSearch:function() {
		...
		// ask server to filter records
			...
			if (store.lastOptions && store.lastOptions.params) {
				delete(store.lastOptions.params[this.paramNames.fields]);
				delete(store.lastOptions.params[this.paramNames.query]);
				delete(store.lastOptions.params['tags']);
				delete(store.lastOptions.params['parent_id']);
			}
			if(bSearch) {
				store.baseParams[this.paramNames.fields] = Ext.encode(fields);
				store.baseParams[this.paramNames.query] = this.viewContext;
			}
			if( this.viewContext.indexOf('search-') > -1 ){
				
				var tagsGrid,tags=[],i;
				if( this.viewContext == 'search-videos-grid'|| this.viewContext == 'search-quizgroups-grid' ){ //ture for lesson and quizzes search 
					
					if( this.grid.id == "course-lesson-grid" ||  this.grid.id == 'course-quizgroups-grid'){ //ture for Course namagement window 
						
						tagsGrid = Ext.getCmp('content-tags-grid');
					}else if( this.grid.id == "videos-grid" ){ //true for Lesson management lessons grid filter 
						
						tagsGrid = Ext.getCmp('lesson-tags-grid');
					}else if( this.grid.id == 'quizgroups-grid'){ //true for Quiz management Quizzes grid filter 
						
						tagsGrid = Ext.getCmp('quiz-tags-grid');
					}
					
				}else if( this.viewContext == 'search-courses-grid' ){ //true for course management Courses grid filter 
					
					tagsGrid = Ext.getCmp('course-tags-grid');
				}else if( this.viewContext == "search-questions-grid" && !Ext.isEmpty(this.grid.parentGrid) ){
					
					var quizSelection = Ext.getCmp(this.grid.parentGrid).getSelectionModel( ).getSelections()[0];
					if( quizSelection ){
						store.baseParams['parent_id'] = quizSelection.get('content_id');
					}else{
						return;
					}
				}
				if( tagsGrid ){
					...
					var selections = tagsGrid.getSelectionModel().getSelections();
					for( i=0; i<selections.length; i++){
						tags.push( selections[i].get('uid') ); 
					}
				}
				
				store.baseParams['tags'] = ( tags.length > 0 ) ? tags.toString() :null;
			}
			// reload store
			store.reload();
		}
...
Ext.preg('pgridsearch',Ext.ux.grid.Search);
// eof
