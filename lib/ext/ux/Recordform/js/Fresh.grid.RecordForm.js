 ...
 * @date      4. April 2008
 * @version   1.0
 * @revision  $Id: Fresh.grid.RecordForm.js 2348 2013-12-20 10:20:54Z arun $
 *
	...
	,createFormConfig:function() {
		...
		if(this.form || !this.grid.store.recordType) {
			return;
                ...
                Fresh.console.log(this.formLayout);

		// get vars
		var cm = this.grid.getColumnModel();
		var fields = this.grid.store.recordType.prototype.fields;
		var store = this.grid.store;
                if (store.meta.recordForm) {
                    this.columnCount = store.meta.recordForm.columnCount || this.columnCount;
                    this.defaultWindowConfig.width = store.meta.recordForm.width || this.defaultWindowConfig.width;
                }
		// {{{
		// create form *config* object - it does NOT instantiate the form
                if (this.formLayout == 'column')
                    this.form = Ext.apply({
                             xtype:'form'
                             ,anchor: '100%'
                            ,items:[{
		...
		var colIndex = 0;
		var tabIndex = 1;
                if (store.meta.formBinding)
                    this.form.formBinding = store.meta.formBinding;
			...
			var extMajorVersion = parseInt(Ext.version);
                        if (f.formEditor) {
                            ...
                            if (c && c.formEditor) {
                                ...
                                if(c && c.editor && (3 === extMajorVersion || c.editor.field) && store.meta.id != f.name && store.meta.formBinding != f.name) {
                                        var editorField = (3 === extMajorVersion && c.editor.getXType()) ? c.editor : c.editor.field;
                                        Ext.apply(o, {
                        ...
                        if (store.meta.id == f.name || store.meta.formBinding == f.name) {
                            o.xtype = 'hidden';
                        }
...
//				o.autoHeight = true;
			...
			// add field to a column on left-to-right top-to-bottom basis
                ...
                if (this.formConfig && this.formConfig.moreButtons)
                    Ext.each(this.formConfig.moreButtons,function(o) {
                        this.form.buttons.push(o);
                    },this);
	 ...
	 */
	,onCancel:function() {
		if(this.record.get('new') && !this.record.dirty) {
	...
	,getPanel:function() {
		...
		panel.on({
			show:{scope:this, delay:this.focusDefer, fn:function() {
				...
				// focus first form field on window show
				basicForm.items.itemAt(0).focus(true,this.focusDefer);

				// mark fields invalid if anysh
				basicForm.isValid();
			}}
			,hide:{scope:this, fn:function() {
				this.form.stopMonitoring();
