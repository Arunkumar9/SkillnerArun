...
class FListMenu extends TListControl implements INamingContainer {
    ...
    protected $_tags;
    protected $_wrap;
    protected $_outer_wrap;
    protected $_generate_item_class;
    protected $_generate_item_class_li;
    protected $_no_base_class_in_a;
    protected $_separator;
    protected $_generate_all;
    protected $_distance;
    protected $_maxdistance=1;
    protected $_generate_linkid;
    ...
    public function getNoBaseClassInA() {
        return $this->_no_base_class_in_a;
    }

    public function setNoBaseClassInA($value) {
        $this->_no_base_class_in_a=TPropertyValue::ensureBoolean($value);
    }

    public function getGenerateItemClassInLi() {
        return $this->_generate_item_class_li;
    }

    public function setGenerateItemClassInLi($value) {
        $this->_generate_item_class_li=$value;
    }
    ...
    public function getGenerateLinkId() {
        return $this->_generate_linkid;
    }

    public function setGenerateLinkId($value) {
        $this->_generate_linkid=TPropertyValue::ensureBoolean($value);
    }
    ...
    public function getOuterWrap() {
        return $this->_outer_wrap;
    }

    public function setOuterWrap($value) {
        $this->_outer_wrap=$value;
    }
    ...
    public function getCurrentUrl() {
        return 	$this->getPage()->Request->getUrl()->getUri();//.'?'.htmlspecialchars($this->getPage()->Request->getUrl()->getQuery());
    }
...
EOD;
    ...
    public function render($writer) {
        ...
        $i = 1;
        while($parent instanceof FListMenu) {
            $this->setActiveCssClass($parent->getActiveCssClass());
            $this->setInactiveCssClass($parent->getInactiveCssClass());
            $this->setGenerateItemClass($parent->getGenerateItemClass());
            $this->setGenerateItemClassInLi($parent->getGenerateItemClassInLi());
            $parent = $parent->parent;
            $i++;
        }
        $ulClass .= $i;
        ...
        if ($this->MaxDistance && is_numeric($this->Distance))
            $ulClass .= ' distance'.$this->Distance;
        if ($this->distance)
            $ulClass .= ' sideMenu'.$i;
        $itemCount = count($this->getItems());
        if (!$this->JustLinks && $itemCount>0)
            $writer->write("\n".'<ul ID="'.$this->ClientID.'" class="'.$ulClass.'">'."\n");
    ...
    protected function renderItems($writer,$depth=1) {
        ...
        foreach($this->getItems() as $item) {
            if ($item instanceof FListMenuItem) {
                ++$ii;
                $lastText = $item->Text;
                //Prado::trace('Page uid '.$currentPagePath,__CLASS__);
                //Prado::trace('Page path item '.$item->getNavigateUrl(),__CLASS__);
                if($item->getContainerID() == $this->getPage()->getContainer()->uid)   //($pagePath=$item->getPagePath())===$currentPagePath && )
                {
                    $current = true;
                ...
                //Prado::trace(__CLASS__.' '.$cssClass,'Json');
                ...
                $LIcssClass = "";
                if ($itemInLi = $this->getGenerateItemClassInLi()) {
                    $LIcssClass = ' '.$itemInLi.$depth.' '.$itemInLi.$depth.'-'.$ii; //$cssClass.' '.
                    $LIcssClass .= ( $ii+1 == count($this->getItems()) )?' '.$itemInLi.$depth.'-last':'';
                }

                if ($item->NextItem instanceof FListMenuItem OR $item->NavigateUrl!='#')
                    $canopen = ' class="'.$cssClass.$LIcssClass.'"';
                else
                    $canopen = ' class="canopen '.$cssClass.$LIcssClass.'"';

                if (!$this->JustLinks)
                    $writer->write("<li $canopen>");
                if ($this->generateItemClass) {
                    if ($this->getNoBaseClassInA())
                        $baseClass = "";
                    else
                        $baseClass = $cssClass." ";
                    $cssClass = $baseClass.self::ITEM_CLASS.$depth.' '.self::ITEM_CLASS.$depth.'-'.$ii; //$cssClass.' '.
                    $cssClass .= ( $ii+1 == count($this->getItems()) )?' '.self::ITEM_CLASS.$depth.'-last':'';
                    $cssClass  = ' class ="'.$cssClass.'"';
                }
                elseif ($cssClass)
                    $cssClass = ' class ="'.$cssClass.'"';
                if ($this->getOuterWrap()) {
                    $outerL = "<".$this->getOuterWrap().">";
                    $outerR = "</".$this->getOuterWrap().">";
                }
                else {
                    $outerL = "";
                    $outerR = "";
                }
                if ($this->getGenerateLinkId())
                    $cID = " ID='".$this->ClientID.'$'.$item->getContainerID()."' ";
                else
                    $cID = '';
                if ($tag = $this->Tags[$depth-1]) {
                    $href = ($tag == 'a')?'href="'.$url.'"':'';
                    $target = ($tag == 'a')?$target:'';
                    $writer->write($outerL."<$tag ".$cID.$href.$cssClass.' '.$target.">".$this->wrapMe($item->getTextWithLineBreaks())."</$tag>".$outerR);
                }
                else
                    $writer->write($outerL."<a $cID href=\"$url\"$cssClass $target>".$this->wrapMe($item->getTextWithLineBreaks())."</a>".$outerR);
                ...
                if (!$this->JustLinks && ($item->NextItem instanceof FListMenuItem || !$item->NextItem))
                    $writer->write("</li>\n");
            }
            else {
