...
Ext.ux.form.SuperBoxSelect = Ext.extend(Ext.ux.form.SuperBoxSelect,Ext.form.ComboBox,{
    ...
    /**
     * @cfg {String} anyMatch If to find any match in store.
     */
    anyMatch : false,
    ...
    initComponent:function() {
       Ext.apply(this, {
        ...
        if(this.mode !== 'remote' && this.store && !this.noStoreClone){
            this.rootStore = (Ext.isString(this.store)) ? Ext.StoreMgr.lookup(this.store) : this.store;
            this.store = this.rootStore;
            if (this.rootStore.fields)
                this.configStore(this.rootStore);
            else {
                this.mon(this.rootStore,'metachange',this.configStore,{ scope: this, single: true});
            }
            this.mon(this.rootStore,'load',this.updateStore,this);
        }
    ...
    configStore: function(store,meta) {
        this.store = new Ext.data.ArrayStore({
            autoDestroy: true,
            storeId: store.storeId+'-local',
            idProperty: 'uid',
            fields: store.fields.clone().getRange()
        });
        if (!meta)
            store.reload();
        return;
    },
    updateStore: function(store,rec,opt) {
        this.store.add(rec);
        this.onStoreLoad(this.store,rec,opt);
        return;
    },
    ...
    manageNameAttribute :  function(only){
    ...
    onEnable: function(){
        ...
        if (this.renderFieldBtns && !this.enabled) {
            this.initButtonEvents();
        }
    },
    onDisable: function(){
...
/*Ext.ux.form.CommonSuperBoxSelect = function(config) {
   myName = config.name;
   Ext.applyIf(config,{
     displayField: 'name'
     ,valueField: 'uid'
     ...
     ,width: '100%'
     ,anchor: '100%'
     ,bindingProperty: 'RelatedDefsAsList'
     ,bindingClass: 'form'
       ,forceAll: true
       ,removeValuesFromStore: false
       ,pinList: false
   });
   Ext.ux.form.CommonSuperBoxSelect.superclass.constructor.call(this,config);
}
Ext.extend(Ext.ux.form.CommonSuperBoxSelect,Ext.ux.form.SuperBoxSelect);

Ext.reg('csuperboxselect', Ext.ux.form.CommonSuperBoxSelect);


Ext.ux.form.CommonBindingSuperBoxSelect = Ext.extend(Ext.ux.form.CommonSuperBoxSelect,{
    initComponent: function() {

       this.plugins = this.plugins || [];
       this.plugins.push('msgbus');
       this.editable = !!this.bindingNotAllowed;
       this.filters = this.filters || {};

       Ext.ux.form.CommonBindingSuperBoxSelect.superclass.initComponent.call(this);

       if (!this.bindingNotAllowed && this.rootStore) {
           this.on('change',this.publishFilter,this);
           //this.on('render',this.laterEvent,this,{single:true});
           this.laterEvent();
       }
    },
    ...
    laterEvent: function() {
           ...
           if (this.form) {
               this.form.getForm().on('actioncomplete',this.publishFilter,this);
               //that = this;
           }
               this.rootStore.on('load',function(s){
                        rcls = this.rootStore.collect('RelatedClassesList',false,true);
                        Fresh.console.log(this.name);
                        for(i=0;i<rcls.length;i++) {
                            cls=rcls[i];
                            for(j=0;j<cls.length;j++) {
                                bname = cls[j];
                                this.subscribe('eu.datarecovery.'+this.bindingClass+'.*.'+bname+'.filter',{fn: this.onFilter, scope: this});
                            }
                        }
    ...
    publishFilter: function(f,newval,oldval) {
       var rec;
       
       var values = this.getValue().split(',');
       for(i=0;i<values.length;i++) {
           var rec = this.rootStore.getById(parseInt(values[i]));
           if (rec) {
                   Fresh.console.log(this.name);
                   this.publish('eu.datarecovery.'+this.bindingClass+'.'+this.getBindingFieldValue()+'.'+rec.get('class')+'.filter',rec.get(this.bindingProperty));
           }
       }
    },

    getBindingFieldValue: function() {
       this.form = this.form || this.findParentByType('form');
       //if (!this.form) rett
       bf = this.bindingField || this.form.formBinding ||  'uid';

       if (this.form)
           field = this.form.getForm().findField(bf);
       //else
           //field = this.
       if (field)
            return field.getValue() || 'new';
       else
           return 'none';

    },
    updateFilter: function(subject,filter) {
        if (subject) {
            bv = this.getBindingFieldValue();
            sa = subject.split('.');
            if (bv == sa[3])
    ...
    getFilter: function(rec,id) {
        f = [];
        bv = this.getBindingFieldValue();

        //ss = 'eu.datarecovery.'+this.bindingClass+'.'+this.getBindingFieldValue();
        //sl = ss.length;
        for(s in this.filters) {
            sf = this.filters[s];
            //sa = sf.split('.');
            if (sf) {
                if (sf.length == 0 || bv != s.split('.')[3]) continue;
    //            f = f.concat(sf);
                f = (f.length==0)?sf:array_intersect(f,sf);
            }
        }
        if (Ext.isArray(f) && f.length>0)
                re = new RegExp('^'+f.join('(000)*$|^')+'(000)*$');
        else
                re = false;
        return re;
    },

    onFilter: function(subject,message) { // eu.datarecovery.order.UID.NAME.filter
        bv = this.getBindingFieldValue();
        sa = subject.split('.');
        if (bv != sa[3])
            return;
        this.updateFilter(subject, message);
        Fresh.console.log('BUS RECEIVED '+this.name+' - '+subject);
        this.doFilter();
    },

    doFilter: function() {
        re = this.getFilter()
        Fresh.console.log(re);
        if (re) {
            
                this.store.filter('uid',re);
                //Fresh.console.log(this.name+' '+values.join(this.valueDelimiter));
            //}
        } else {
            this.store.clearFilter();
        }
            ...
            if(this.lastQuery !== q || forcedAdd){
                ...
                if(this.mode == 'local'){
                    ...
                    if(forceAll){
                        ...
                        this.doFilter(); //this.store.filter(this.displayField, q, this.anyMatch);
                    }
                    this.onLoad();
                }else{
...
Ext.reg('cbsuperboxselect', Ext.ux.form.CommonBindingSuperBoxSelect);
*/
Ext.ux.form.CommonSuperBoxSelect = function(config) {
   myName = config.name;
   Ext.applyIf(config,{
...
Ext.ux.Plugin.DragDropSuperBoxSelect = function(config) {
    Ext.apply(this, config);
    Ext.ux.Plugin.DragDropSuperBoxSelect.superclass.constructor.call(this);
};
Ext.extend(Ext.ux.Plugin.DragDropSuperBoxSelect,Ext.util.Observable,{

   // maxDroppedNodes: 0,

    init: function(c) {
        c.maxDroppedNodes = this.maxDroppedNodes;
        c.onRender = c.onRender.createSequence(this.onRender,c);
    },
    onRender: function(ct,position) {
          var c = this;
          var dtEl = c.wrapEl.dom;
          c.dtField = new Ext.dd.DropTarget(dtEl, {
                          ddGroup: c.ddGroup || 'tree-containers',
                          notifyEnter: function(ddSource, e, data) { c.wrapEl.highlight(); },
                          notifyDrop: function(ddSource, e, data) {
                            var node = data.node;
                            var rec,up = this.maxDroppedNodes;

                            //Fresh.console.log(node);
                            while (node && node.id != 'root') {
                              up = up - 1;
                              if ((rec = c.store.getById(node.id)) ) {
                                c.addRecord(rec);
                                if (up == 0) return(true);
                              } else {
                                return(true);
...
Ext.preg('pddsuperboxselect',Ext.ux.Plugin.DragDropSuperBoxSelect);