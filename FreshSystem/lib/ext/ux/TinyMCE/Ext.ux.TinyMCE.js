...
(function() {
	...
	*/
	...
	// Members
		...
		*/
		initComponent: function(c) {
			...
			Ext.ux.TinyMCE.initTinyMCE({language: this.tinymceSettings.language});
                        Ext.ux.TinyMCE.superclass.initComponent(this,c);
		},
		...
		*/
		initEvents: function() {
			this.originalValue = this.getValue();
                        //Ext.ux.TinyMCE.superclass.initEvents();
		},
		...
		*/
		onRender: function(ct, position) {
			...
			this.ed.onPostRender.add(function(ed, controlManager) {
				...
				Ext.Element.fly(s.content_editable ? ed.getBody() : ed.getWin()) //(tinymce.isGecko ? ed.getDoc() : ed.getWin()))
		...
		*/
		onResize: function(aw, ah) {
			if( Ext.type( aw ) != "number" ){
				aw = this.getSize().width;
			}
			if( Ext.type(ah) != "number" ){
			//	ah = this.getSize().height;
			}
			if (aw == 0 || ah == 0)
				return;
		...
		*/
		_setEditorSize: function( width, height ) {
			...
			if ('function' === typeof this.ed.theme.resizeTo)
		...
		*/
		validateValue: function( value ) {
			if('function' == typeof this.validator){
	...
	Ext.ComponentMgr.registerType("tinymce", Ext.ux.TinyMCE);
        ...
        Ext.ux.CommonTinyMCE = function(config) {
            ...
            config.tinymceSettings = {
                ...
                'theme_advanced_buttons2' : "search,replace,|,bullist,numlist,|,outdent,indent,blockquote,|,undo,redo,|,link,unlink,anchor,image,cleanup,help,code,|,hr,sub,sup,|,charmap,fullscreen,template",
                 //plugins: "safari,pagebreak,style,layer,table,advhr,emotions,iespell,advimage,advlink,insertdatetime,preview,media,searchreplace,print,contextmenu,paste,directionality,noneditable,visualchars,nonbreaking,xhtmlxtras,template",
                 //theme_advanced_buttons1:  "bold,italic,underline,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull,|,formatselect,tablecontrols,|,cut,copy,paste,pastetext,pasteword",
                 //theme_advanced_buttons2:  "search,replace,|,bullist,numlist,|,outdent,indent,blockquote,|,undo,redo,|,link,unlink,anchor,image,cleanup,help,code,|,hr,sub,sup,|,charmap",
            ...
            Ext.ux.CommonTinyMCE.superclass.constructor.call(this,config);
        }


        Ext.extend(Ext.ux.CommonTinyMCE,Ext.ux.TinyMCE, {
        });


        Ext.ComponentMgr.registerType("ctinymce", Ext.ux.CommonTinyMCE);
	...
	*/
    ...
    if (type == 'file') {
        ...
        var chooser;
        if (Fresh.localParameters && Fresh.localParameters.defaultImageSize)
            chooser = new ImageChooser({
                        width: 515,
                        height: 400,
                        defaultImageSize: Fresh.localParameters.defaultImageSize
                        });
        else
            chooser = new ImageChooser({
                        width: 515,
