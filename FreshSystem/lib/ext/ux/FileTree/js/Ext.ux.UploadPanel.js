 ...
 * @version $Id: Ext.ux.UploadPanel.js 2348 2013-12-20 10:20:54Z arun $
 * @date    13. March 2008
 *
...
//					+ '<div class="ux-up-indicator">&#160;</div>'
		...
		this.uploader = new Ext.ux.file.Uploader(config);
		...
		this.on({
			...
			,uploadprogress:{fn:this.onProgress.createDelegate(this)}
		});
	 ...
	 * @private
            ...
            ,onAddFile:function(fu,fileName) {
                ...
                fu.detachFileInput();
                var fileName,inps;
                var files = inp.dom.files;
                if (files) {

                }
                else {
                    files = [{name: this.getFileName(inp)}];
                }
		...
		// create new record and add it to store
			...
			,fileCls:this.getFileCls(fileName)
                        ...
                        type: (file.type ? file.type : file.fileType) || this.getFileCls(fileName), // missing if safari and chrome
	 ...
	 */
	,onProgress:function(uploader, record,e) {
		var progress, state, idx, item, width, pgWidth;
		//Fresh.console.log(uploader);
		...
		//Fresh.console.log(e);
			...
			if(item) {
				width = item.getWidth();
				item.applyStyles({'background-position':width * progress / 100 + 'px'});
			}
		}
