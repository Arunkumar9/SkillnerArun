 ...
 * @version $Id: FRedisCache.php 2348 2013-12-20 10:20:54Z arun $
 * @package System.Caching
 * @since 3.0
 */
class FRedisCache extends TCache
{
	/**
	...
	//		throw new TConfigurationException('phpredis_extension_required');
	if(extension_loaded('newrelic')) { 
		newrelic_add_custom_tracer ('FRedisCache::setValue');
		newrelic_add_custom_tracer ('FRedisCache::getValue');
	}	
...
/*		$this->loadConfig($config);
	...
	public function connect() {
		...
		if ($this->_persistence)
			$this->_cache->pconnect($this->_host,$this->_port,$this->_timeout);
		else
			$this->_cache->connect($this->_host,$this->_port,$this->_timeout);
		
		
		if ($this->_prefix) {
			$this->_cache->setOption(Redis::OPT_PREFIX, $this->_prefix);
		}
		
		$this->_cache->setOption(Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP);
		
		if ($this->_db)
			$this->_cache->select($this->_db);
	}
	...
	public function setPrefix($value)
	{
		if($this->_initialized)
			throw new TInvalidOperationException('redis_prefix_unchangeable');
		else
			$this->_prefix=$value;
	}
	...
	{
		if($this->_initialized)
			throw new TInvalidOperationException('redis_db_unchangeable');
		else
			$this->_db= (int) $value;
	}
	...
	/**
	 * @param string host name of the redis server
	 * @throws TInvalidOperationException if the module is already initialized
	 */
	public function setHost($value)
	{
		if($this->_initialized)
			throw new TInvalidOperationException('redis_host_unchangeable');
		else
			$this->_host=$value;
	}

	/**
	 * @return integer port number of the redis server
	 */
	...
	/**
	 * @param integer port number of the redis server
	 * @throws TInvalidOperationException if the module is already initialized
	 */
	public function setPort($value)
	{
		if($this->_initialized)
			throw new TInvalidOperationException('redis_port_unchangeable');
		else
	...
	{
		//var_dump($this->_cache->get($key));die();
	...
	{
		$res =  $this->_cache->setnx($key,$value);
		if ($res)
	...
	public function flush()
	{
		...
		foreach($keys as $k) {
			...
			$this->_cache->delete(str_replace($this->_prefix,'',$k));
		}
		return true;
	}
