...
Ext.ux.form.TreeComboSuperBox = Ext.extend( Ext.form.TriggerField, {
  ...
  manageNameAttribute :  function(only){
    ...
    initEvents : function() {
        ...
        Ext.ux.form.TreeComboSuperBox.superclass.initEvents.call(this);
    ...
    onFocus: function() {
        ...
        Ext.ux.form.TreeComboSuperBox.superclass.onFocus.call(this);
    },
    ...
    onBlur: function() {
        ...
        Ext.ux.form.TreeComboSuperBox.superclass.onBlur.call(this);
    },
    collapse: function() {
        this.treePanel.hide();
    },
    ...
    setValue: function( value ) {
		...
		if (!this.treePanel.rendered) {
			this.treePanel.on('afterrender',this.setValue.createDelegate(this,[value]), this, {single: true});
			return;
		}
	/*	
		var nc=0; var hash = this.treePanel.nodeHash;
		for(var k in hash) if(hash.hasOwnProperty(k)) nc++;
		if (nc==1) {
			this.treePanel.expandAll();
			this.setValue.defer(2000,this,[value]);
			return;
		}
	*/    
		this.removeAllItems();
        ...
        if( this.multiSelect ) {
	        Ext.each( values, function( val ) {
            	//var node = this.treePanel.getNodeById(val);
            	//var node = this.treePanel.root.findChild('id',val,true);
                //if (!node) return;
                var itemVal = val;
		        var itemDisplay;
				var rec = this.findInStore(itemVal);
				if (rec)
					itemDisplay = rec.get(this.displayField);
				var itemCaption = itemDisplay; //this.getCaption( node );
		        var itemClass = '';
		        var itemStyle = '';
 ...
 */
Ext.ux.form.CommonTreeComboSuperBox = function(config) {
   config = config || {};
   myName = config.name || 'category';
   Ext.applyIf(config,{
     displayField: 'name'
     ,valueField: 'uid'
     ,mode: 'local'
	 ...
	 ,treeConfig: {
        ...
        loader: {
					...
					baseParams: { view: 'tree-combo-containers' },
   ...
   Ext.ux.form.CommonTreeComboSuperBox.superclass.constructor.call(this,config);
}



Ext.extend(Ext.ux.form.CommonTreeComboSuperBox,Ext.ux.form.TreeComboSuperBox);/*,{
...
Ext.reg('ctreecombosuperbox', Ext.ux.form.CommonTreeComboSuperBox);