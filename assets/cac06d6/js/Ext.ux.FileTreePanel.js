 ...
 * @version $Id: Ext.ux.FileTreePanel.js 2348 2013-12-20 10:20:54Z arun $
 * @date    13. March 2008
 *
 ...
 *---------------------------------------------------------------------------------------------------------------------------------------------------
 * 27645		   Arunkumar.muddada  201405190301      Modified : showAwesomeUplaoder
 * 														AwesomeUplaoder help description is showing for only lesson management Files(movies),Video Captions(captionfiles) tabs.
 * 
 * 														While preparation of the upload window we are passing the help window parameters so
 * 														AwesomeUploaderPop will use this parameters and prepare a help window.
 * 
 * 29024		   Dinesh GK		  201407190430		modified : For questions images tab allow to show only images files not for all file formats.
 */
...
Ext.ux.FileTreePanel = Ext.extend(Ext.tree.TreePanel, {
	...
	,initComponent:function() {
		...
		if(!this.loader) {
			...
			var baseParams = Ext.apply({},this.baseParams);
			baseParams.cmd = 'get';
	...
	,getContextMenu:function() {
		...
		if(!this.contextmenu) {
			var config = {
				...
				,enableProgress:this.enableProgres
			};
			if(this.baseParams) {
			...
			this.uploadPanel = this.contextmenu.getItemByCmd('awesomeuploader'); 
			this.uploadPanel.config = config.baseParams;
			this.uploadPanel.config.url = this.url;
...
//			this.uploadPanel = this.contextmenu.getItemByCmd('upload-panel'); 
	...
	,onContextClick:function(menu, item, e) {
		...
		switch(item.cmd) {
			...
			case 'newdir':
				this.createNewDir(node);
			break;
			case 'awesomeuploader':
				this.showAwesomeUplaoder(menu);
			break;
	...
	,showContextMenu:function(node) {
		...
		menu.setItemDisabled('expand', true || node.isLeaf());
		menu.setItemDisabled('collapse', true || node.isLeaf());
		...
		menu.getItemByCmd('upload').setVisible(false && this.enableUpload);
	...
	// }}}
	,showAwesomeUplaoder :function( menu ){
		
		var sm = this.getSelectionModel();
		var node = sm.getSelectedNode();
		var nodePath = this.getPath(node);
		var params = menu.baseParams;
		params.nodepath = nodePath;
		var uploadFileTypes;
		
		//201405190301
		var showHelpDescription = false ;
		var helpDescription ="";
	    //AwesomeUplaoder help description is showing for only lesson management Files(movies),
		//Video Captions(captionfiles) tabs.
		if( params.panel == 'movies' || params.panel == 'captionfiles' || params.panel == 'images' ){
			showHelpDescription = true;
			if(params.panel == 'movies') {
				helpDescription = AWESOME_HELP_DESCRIPTION.MOVIES;
				uploadFileTypes = '*.mp4';
			} else if(params.panel == 'captionfiles'){
				helpDescription = AWESOME_HELP_DESCRIPTION.CAPTION;
				uploadFileTypes = '*.srt';
			}else if(params.panel == 'images'){ //#29024  201407190430
				uploadFileTypes ='*.png;*.jpg;*.jpeg;*.gif;*.tif;*.tiff;*.bmp';
			}
		}
		var options = {
				 url:params.url
				...
				,success: function(response, opts) {
					...
					var path = response.responseText.replace( /"/gi, '').replace( /\\/gi, '');;
					if( params.nodepath == "root"){
						
						path = path;
					}else{
						path = path+params.nodepath.split('root/')[1]+"/";
					}
					menu.setVisible(false);
					//201405190301
					//While preparation of the upload window we are passing the help window parameters so 
					// AwesomeUploaderPop will use this parameters and prepare a help window.
					var uploadWindow = new AwesomeUploaderPopup({
						panel:params.panel,
						path :params.url,//FJsonFileProvider path
						cmd:params.cmd,
						dir:params.dir,
						uploadPath : path, // File Uploader upload path 
						fileTreePanel :menu.node,
						isfromamazon : true,
						closable:false,
						y:30,
						showHelpDescription : showHelpDescription, 		// True to make the Help Decription show in toolbar,false to hidde description.
						helpPosition : 'bottom',					// Help description postion 'bottom' to show in fbar, 'top' to show in tbar.
						helpDescription :helpDescription,	// Help Description is string to show html content in Help Description panel.
